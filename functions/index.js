...
exports.dailyWateringNotification = onSchedule(
  ...
  async () => {
    ...
    if (!enabled || !recipientEmail || !senderEmail) return
    ...
    // Load watering log (last 90 days)
    ...
    // Mirror the frontend isDueToday logic
    ...
    function isDue(plant) {
      ...
      return daysSince >= plant.wateringIntervalDays
    }
    ...
    const duePlants = plants.filter(isDue).sort((a, b) => a.number - b.number)
    if (!duePlants.length) return // nothing due — no email needed
    ...
    const plantRows = duePlants
      .map(
        p => `
      <tr>
        <td style="padding:10px 14px;border-bottom:1px solid #eee;">
          <strong style="color:#1a1a1a;">#${p.number} ${p.name}</strong><br>
          <span style="font-size:12px;color:#888;">Last watered: ${fmtLastWatered(p)}</span>
        </td>
      ...
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a;">
        ...
        <div style="background:#fff;padding:20px 24px;border:1px solid #eee;border-top:none;">
          <p style="margin:0 0 16px;font-size:15px;">
            ${duePlants.length === 1
              ? `<strong>1 plant</strong> needs water today:`
              : `<strong>${duePlants.length} plants</strong> need water today:`}
          </p>
    ...
    await transporter.sendMail({
      ...
      to: recipientEmail,
      subject: `🌿 ${duePlants.length} plant${duePlants.length > 1 ? 's' : ''} need water today — ${dateStr}`,
      html,
    })

    console.log(`Notification sent to ${recipientEmail} for ${duePlants.length} plants`)
  }
)
